...
-- First, let's see what policies currently exist on the suppliers table
...
-- Admin full access
CREATE POLICY "Suppliers: Admin full access" 
...
-- Authenticated users can only view basic supplier info (NO contact details)
CREATE POLICY "Suppliers: Authenticated basic view only" 
...
BEGIN
  ...
  SELECT 
    ...
    s.updated_at,
    CASE 
      WHEN is_authenticated THEN 'Contact available to registered users'
...
BEGIN
  ...
  -- Determine contact access permissions with strict business relationship requirements
...
CREATE POLICY "Supplier audit: Admin access only" 
...
-- Enhanced logging function with audit trail
CREATE OR REPLACE FUNCTION public.log_supplier_contact_access_enhanced(
  supplier_uuid uuid, 
...
AS $$
BEGIN
  -- Insert into both the existing log and the new audit table
  ...
  -- Insert into enhanced audit table
  ...
  VALUES (
    ...
    supplier_uuid,
    access_type_param,
    access_type_param LIKE '%authorized%',
    business_justification,
    fields_accessed
  );
