...
-- Drop ALL existing policies on deliveries table to ensure clean slate
...
CREATE POLICY "Secure deliveries: Status updates only" 
...
CREATE POLICY "Secure deliveries: Builder creation" 
...
-- Enhanced security function for protected driver info access
...
BEGIN
  ...
  -- Strict driver contact access permissions
  ...
  -- Log all driver info access attempts
  INSERT INTO driver_info_access_log (
    ...
    delivery_uuid, 
    CASE 
      WHEN can_access_driver THEN 'secure_driver_info_authorized_' || access_reason
      ELSE 'secure_driver_info_denied_' || COALESCE(user_profile_record.role, 'unknown')
    END
  );
...
-- Update the existing secure listings function
